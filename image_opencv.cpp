...
extern "C" void draw_detections_cv_v3(mat_cv* mat, detection *dets, int num, float thresh, char **names, image **alphabet, int classes, int ext_output)
{
    try {
        ...
        for (i = 0; i < num; ++i) {
			...
			// yhsw -->
			// MOTIFY VIDEO ONLY CAR
			if (j == 2) { // j == 2 is car, 7 is truck
		...
		printf("b_x_center: %d, b_y_center: %d, b_width: %d, b_height: %d\n", b_x_center, b_y_center, b_width, b_height);


		box center_box = dets[i].bbox;
		center_box.x = 640;
		center_box.y = 520;
		center_box.w = 350;
		center_box.h = 240;
		...
		box center_box_2 = dets[i].bbox;
                center_box_2.x = b_x_center;
                center_box_2.y = b_y_center;
                center_box_2.w = b_width;
                center_box_2.h = b_height;

		float iou_temp = 0.0;
		
		iou_temp = box_iou(center_box, center_box_2);
		printf("IoU = %.2f\n", iou_temp);

		if (iou_temp > 0.15 && class_id == 2) {
			printf("WARNING!!! TOO CLOSE!!!\n");
		}

		if (iou_temp > 0.2 && class_id == 7) {
                        printf("WARNING!!! TOO CLOSE!!!\n");
                }
		...
		// center box, 
		int new_left = 465, new_right = 815, new_top = 400, new_bot = 640;
		cv::Point Center1, Center2, Center_pt_text, Center_pt_text_bg1, Center_pt_text_bg2;
		cv::Scalar color_red, color_blue;
		cv::Scalar black_color = CV_RGB(0, 0, 0);
		color_red.val[0] = 0;
		color_red.val[1] = 0;
		color_red.val[2] = 256;
		color_blue.val[0] = 256;
		color_blue.val[1] = 0;
		color_blue.val[2] = 0;
		Center1.x = new_left;
		Center1.y = new_top;
		Center2.x = new_right;
		Center2.y = new_bot;
		Center_pt_text.x = new_left;
		Center_pt_text.y = new_top - 4;
		Center_pt_text_bg1.x = new_left;
		//Center_pt_text_bg1.y = 385 - (3 + 18 * font_size);
		Center_pt_text_bg1.y = new_top - 10;
                Center_pt_text_bg2.x = new_right;
                if ((new_right - new_left) < text_size.width) Center_pt_text_bg2.x = new_left + text_size.width;
                Center_pt_text_bg2.y = new_top;
		


		cv::rectangle(*show_img, Center1, Center2, color_blue, width, 8, 0);  // draw box


		
		// draw warning sign
		if (iou_temp >= 0.15 && class_id == 2) {
		cv::rectangle(*show_img, Center_pt_text_bg1, Center_pt_text_bg2, color_red, width, 8, 0);
                cv::rectangle(*show_img, Center_pt_text_bg1, Center_pt_text_bg2, color_red, CV_FILLED, 8, 0);    // filled
		cv::putText(*show_img, "WARNING!!! TOO CLOSE!!!", Center_pt_text, cv::FONT_HERSHEY_COMPLEX_SMALL, font_size, black_color, 2 * font_size, CV_AA);
		}
		...
		// yhsw back color declare above by me,
		// so, you can mark the next raw
                //cv::Scalar black_color = CV_RGB(0, 0, 0);
